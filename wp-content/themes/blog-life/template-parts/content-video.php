...
<article id="post-<?php the_ID(); ?>" <?php post_class($blog_life_clearfix_class); ?>>
  <?php
  // Check for video
    $blog_life_videopost_content = apply_filters( 'the_content', get_the_content() );
    $blog_life_video = false;

    // Only get video from the content if a playlist isn't present.
    if ( false === strpos( $blog_life_videopost_content, 'wp-playlist-script' ) ) {
      $blog_life_video = get_media_embedded_in_content( $blog_life_videopost_content, array( 'video', 'object', 'embed', 'iframe' ) );
    }

    // check for feature image
    if(!empty( $blog_life_video ) ){ 
      foreach ( $blog_life_video as $blog_life_video_html ) {
        echo '<a class="featured-image-link video">';
        echo $blog_life_video_html;
        echo '</a>';
      } 
